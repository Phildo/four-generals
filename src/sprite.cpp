...
void Sprite::init(const char *f)
{
  ...
  while(fgets(buff, 255, (FILE*)fp))
  {
    ...
    else //alphanum
    {
      ...
      parseIntoRect(&buff[i+1],&alpha[c]);
    }
  }
  fclose(fp);
}

void parseIntoRect(char *b, SDL_Rect *v)
{
  j = i;
  while(buff[i] != ',') i++; buff[i] = '\0';
  v->x = String(buff[i]).intVal();
  j = i;
  while(buff[i] != ',') i++; buff[i] = '\0';
  v->y = String(buff[i]).intVal();
  j = i;
  while(buff[i] != ',') i++; buff[i] = '\0';
  v->w = String(buff[i]).intVal();
  j = i;
  while(buff[i] != ',') i++; buff[i] = '\0';
  v->h = String(buff[i]).intVal();
}
